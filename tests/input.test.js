...
inputClock += 800;
G.input.update();
assert.equal(G.input.tapped("pageRight"), false, "holding R1 must not race across several menu pages");
...
inputClock += 800;
G.input.update();
assert.equal(G.input.tapped("pageRight"), false, "holding R2 must advance only one menu page per pull");
pad.buttons[7] = gamepadButton();
G.input.update();
