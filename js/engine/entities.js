...
G.updatePlayer = function (dt) {
  ...
  const worldInteraction = (G.openingInteractionCandidate && G.openingInteractionCandidate()) || (G.legendEchoCandidate && G.legendEchoCandidate()) ||
...
G.updateBossHazards = function (dt) {
  ...
  for (let i = hazards.length - 1; i >= 0; i--) {
    ...
    if(h.kind === "riftVolley"){
      if(!h.fired){h.fired=true;fireRiftbladeVolley(h.owner,{x:h.x,y:h.y});}
      continue;
    }
...
function updateBossState(e, p, dist, dt) {
  ...
  if (G.updateOrchardBoss && boss.orchard) return G.updateOrchardBoss(e, p, dt);
      ...
      if (beh === "shooter" && d < (e.def.aggro || 110)) {
        ...
        if (e.shootT <= 0) {
          ...
          if (e.def.miniboss && e.bossPhase >= 2) {
            ...
            enemyShot(s, e, a, { damage: e.def.damage || 1, speed: e.def.projectileSpeed || 90, range: 140 });
          }
        }
      }
...
G.drawEnemy = function (ctx, e) {
  ...
  // full stride cycle.
  const animationMode = e.def.miniboss && (e.bossTelegraphT > 0 || e.bossChargeT > 0 || (e.def.boss.orchard && (G.state.openingHazards || []).some(h => h.owner === e && h.t < h.warn))) ? "attack" : "walk";
