...
(() => {
  ...
  const savedMap = G.state.expeditionRun
    ...
    : save && save.mapId === "manyfoldExpedition"
      ...
      : save && G.maps[save.mapId] ? save.mapId : !save && G.maps.orchardRoad ? "orchardRoad" : "overworld";
  ...
  function draw() {
    ...
    const drawables = s.enemies.filter((e) => !e.dead && !e.def.practice).map((e) => ({ y: e.y, fn: () => G.drawEnemy(ctx, e) }));
    ...
    if (G.openingDrawables) drawables.push(...G.openingDrawables(ctx));
    drawables.push({ y: p.y, fn: () => G.drawPlayer(ctx) });
    drawables.sort((a, b) => a.y - b.y);
    for (const d of drawables) d.fn();

    if (G.drawOpeningAtmosphere) G.drawOpeningAtmosphere(ctx, { x: camX, y: camY });
    G.drawAimGuide(ctx);
