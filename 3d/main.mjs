import {Soundscape} from './audio.mjs?v=20260907-battle';
import {portrait,DialogueText} from './dialogue.mjs?v=20260907-battle';
import {installPointerControls,installGameplayGestures} from './touch-controls.mjs?v=20260907-battle';
import {Game,readProgress,saveProgress,freshProgress} from './game.mjs?v=20260907-battle';
import {WorldView} from './scene.mjs?v=20260907-battle';
import {WEAPONS,LANDMARKS,STRUCTURES,ROADS,ENEMIES,SAVE_KEY,landHeight,region,clamp,distance} from './world-data.mjs?v=20260907-battle';
...
function nextLine(){const line=dialogueLines.shift()||'';dialogueText.set(line,performance.now()/1000,matchMedia('(prefers-reduced-motion: reduce)').matches);$('#speech-full').textContent=line;$('#speech').textContent=dialogueText.visible(performance.now()/1000);}
function speak(speaker,lines){$('#speaker').textContent=speaker;$('#crew-portrait').innerHTML=portrait(speaker);$('#speaker-role').textContent=({'Sera Vale':'PILOT · LAST ANCHORAGE','Iona Rusk':'SIGNAL KEEPER','Oren Flint':'ENGINE WRIGHT'})[speaker]||'FROM THE CREW JOURNAL';dialogueLines=[...lines];nextLine();openDialog('#dialogue');}
function nextSpeech(){if(!dialogueText.complete(performance.now()/1000)){dialogueText.reveal();return;}if(dialogueLines.length)nextLine();else closeDialogs();}
...
try{
  ...
  let last=performance.now();function frame(now){try{const dt=Math.min((now-last)/1000,.05);last=now;const pad=gamepad(dt);if($('#dialogue').open){$('#speech').textContent=dialogueText.visible(now/1000);$('#continue').innerHTML=dialogueText.complete(now/1000)?(dialogueLines.length?'Read on':'Until next time')+' <span aria-hidden="true">➜</span>':'Reveal <span aria-hidden="true">➜</span>';}
    if(!paused()){
