...
function ui(now){const p=game.player,w=WEAPONS[p.weapon],q=game.quest();$('#health').style.width=`${Math.max(0,p.hp/game.maxHP*100)}%`;$('#health-label').textContent=`${Math.ceil(p.hp)} / ${game.maxHP}`;$('#pressure').style.width=`${p.charge}%`;$('#pressure-label').textContent=`${Math.floor(p.charge)}%`;$('#pressure-note').textContent=p.charge>=game.ventCost?`VENT READY / COST ${game.ventCost}`:'LAND HITS TO BUILD PRESSURE';$('#quest-title').textContent=q.title;$('#quest-detail').textContent=q.detail;$('#quest-count').textContent=q.count;$('#attack-name').textContent=p.weapon==='maul'?'Crush':p.weapon==='pike'?'Thrust':'Cut';$('#vent-name').textContent=w.vent;$('#attack i').style.transform=`scaleY(${p.attack?1-p.attack.age/p.attack.duration:0})`;$('#vent i').style.transform=`scaleY(${p.charge<game.ventCost?1-p.charge/game.ventCost:0})`;$('#dodge').style.opacity=p.dodge>0?'.5':'1';for(const b of $$('[data-weapon]')){b.classList.toggle('selected',b.dataset.weapon===p.weapon);b.setAttribute('aria-pressed',String(b.dataset.weapon===p.weapon));}
 const near=!paused()?game.nearby():null;$('#interact').classList.toggle('hidden',!near);if(near){$('#interact-label').textContent=near.name;$('#interact .key').textContent=padPresent?'Y':'E';}const boss=game.enemies.find(e=>e.type==='harrow'&&game.active(e)&&distance(e,p)<27)||game.enemies.find(e=>e.type==='engine');const bossData=ENEMIES[boss.type];$('#boss>span').textContent=bossData.name.toUpperCase();$('#boss').classList.toggle('hidden',!(game.active(boss)&&distance(boss,p)<24));$('#boss-health').style.width=`${Math.max(0,boss.hp/bossData.hp*100)}%`;$('#boss-posture').textContent=boss.state==='stagger'?'CORE EXPOSED / STRIKE NOW':`POSTURE ${Math.round(boss.posture/bossData.posture*100)}% / BREAK WITH HEAVY BLOWS`;
