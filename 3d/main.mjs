import {REGULATOR_LINES} from './prologue.mjs?v=20260908-last-light-reviewed';
import {Soundscape} from './audio.mjs?v=20260908-last-light-reviewed';
import {portrait,DialogueText} from './dialogue.mjs?v=20260908-last-light-reviewed';
import {installPointerControls,installGameplayGestures} from './touch-controls.mjs?v=20260908-last-light-reviewed';
import {Game,readProgress,saveProgress,freshProgress,restartChapter} from './game.mjs?v=20260908-last-light-reviewed';
import {WorldView} from './scene.mjs?v=20260908-last-light-reviewed';
import {WEAPONS,LANDMARKS,STRUCTURES,ROADS,ENEMIES,SAVE_KEY,landHeight,region,clamp,distance} from './world-data.mjs?v=20260908-last-light-reviewed';

const preview=new URLSearchParams(location.search).get('preview')==='last-light';
...
if(preview){try{const session=window.sessionStorage;storage={getItem:k=>session.getItem('preview.'+k),setItem:(k,v)=>session.setItem('preview.'+k,v)};}catch{const values=new Map();storage={getItem:k=>values.get(k),setItem:(k,v)=>values.set(k,v)};}}
...
let pointerControls,gameplayGestures,dialogueComplete=null,dialogueBeats=[];
...
const save=()=>{const ok=saveProgress(storage,game);$('#save-status').textContent=ok?(preview?'Opening preview · main save preserved':'Saved on this device'):'Saving unavailable in this browser';lastSaved=game.time;return ok;};
...
const dialogueText=new DialogueText();
function setSpeaker(speaker){$('#speaker').textContent=speaker;$('#crew-portrait').innerHTML=portrait(speaker);$('#speaker-role').textContent=({'Veyr':'SALVAGE RUNNER','Sera Vale':'PILOT · LAST ANCHORAGE','Iona Rusk':'SIGNAL KEEPER','Oren Flint':'ENGINE WRIGHT'})[speaker]||'FROM THE CREW JOURNAL';}
function nextLine(){const beat=dialogueBeats.shift()||{speaker:'Sera Vale',text:''};setSpeaker(beat.speaker);dialogueText.set(beat.text,performance.now()/1000,matchMedia('(prefers-reduced-motion: reduce)').matches);$('#speech-full').textContent=beat.text;$('#speech').textContent=dialogueText.visible(performance.now()/1000);}
function speak(speaker,lines=[],complete=null,beats=null){dialogueComplete=complete;dialogueBeats=beats?[...beats]:lines.map(text=>({speaker,text}));if(!dialogueBeats.length)dialogueBeats=[{speaker,text:''}];nextLine();openDialog('#dialogue');}
function nextSpeech(){if(!dialogueText.complete(performance.now()/1000)){dialogueText.reveal();return;}if(dialogueBeats.length){nextLine();return;}const complete=dialogueComplete;dialogueComplete=null;closeDialogs();if(complete==='last-light-briefing')game.finishPrologueBriefing();}
function paused(){return !started||!!$('dialog[open]')||view.overview||document.hidden;}
function start(){if(started)return;initAudio();started=true;view.started=true;$('#welcome').classList.add('hidden');$('#hud').classList.remove('hidden');$('#start').blur();lastRegion='';if(game.progress.prologue==='arrival')game.beginPrologue();save();}
...
function drawMap(){
  ...
  const pieces=game.progress.teeth;$('#regulator-chart').hidden=!game.progress.briefed;
  for(const [i,id] of ['west','north','east'].entries())$('#regulator-segment-'+i).classList.toggle('seated',pieces.includes(id));
  $('#regulator-count').textContent=game.progress.won?'Engine steady':pieces.length+' / 3 pieces fitted to your suit';
  $('#regulator-note').textContent=pieces.length?REGULATOR_LINES[pieces.length-1]:'The three pieces keep the engine’s power steady. Carry them in your suit to the engine controls.';
}
...
function wire(){
  ...
  $('#restart').onclick=()=>openDialog('#reset-dialog');$('#confirm-reset').onclick=()=>{clearInput();game=restartChapter(game);view.game=game;view.overview=false;view.resetPresentation?.();closeDialogs();if(!started)start();lastRegion='';lastQuest='';toastQueue=[];toastUntil=captionUntil=chapterUntil=feedbackUntil=0;dialogueComplete=null;dialogueBeats=[];lastSaved=0;save();};
...
let feedbackUntil=0,questUntil=0,lastQuest='',captionUntil=0,chapterUntil=0;
function events(){for(const e of game.events.splice(0)){sound(e);view.prologueEvent?.(e.type,e);if(e.type==='save')save();if(e.type==='toast')toast(e.text);if(e.type==='dialogue')speak(e.speaker,e.lines,e.complete,e.beats);if(e.type==='equip')save();if(e.type==='hit')view.shake=Math.max(view.shake,e.heavy?.8:.35);if(e.type==='damage')view.shake=.65;if(e.type==='stagger'||e.type==='perfect'||e.type==='precision'){$('#feedback').textContent=e.type==='precision'?'True point':e.type==='perfect'?'Perfect evade':'Core exposed';feedbackUntil=performance.now()/1000+.8;}if(e.type==='respawn')toast('Sera recovered your rig. Recovered parts and salvage are safe.',4);if(e.type==='salvage')toast(`Rig reinforced. +10 maximum health. ${e.count} / 3 survey cases.`,4);if(e.type==='relay-start')toast('Transmitter active. First patrol incoming. Clear both patrols, then secure it.',5);if(e.type==='relay-wave')toast('Second patrol incoming. Keep the transmitter standing.',4);if(e.type==='relay-secured')toast(e.count===2?'Both relays online. The Tempest Harrow is exposed in the northwest.':`Relay online. ${e.count} / 2. Rig repaired.`,5);if(e.type==='rescue-victory')toast('The stormline is clear. Return to Iona for your upgraded regulator.',6);if(e.type==='crew-home')toast('CREW REGULATOR FITTED / Vent cost 20 / +20 health',5);if(e.type==='victory')toast('Regulator freed. Return to Sera. Bring the crews home.',6);if(e.type==='caption'){caption(e.speaker,e.text);}if(e.type==='chapter'){chapter(e.title,e.subtitle);}}}
function caption(speaker,text){$('#caption-speaker').textContent=speaker;$('#caption-text').textContent=text;captionUntil=performance.now()/1000+Math.max(5.5,text.length/17);}
function chapter(title,subtitle){$('#chapter-title').textContent=title;$('#chapter-subtitle').textContent=subtitle;chapterUntil=performance.now()/1000+4.2;}
function ui(now){const p=game.player,w=WEAPONS[p.weapon],q=game.quest();$('#health').style.width=`${Math.max(0,p.hp/game.maxHP*100)}%`;$('#health-label').textContent=`${Math.ceil(p.hp)} / ${game.maxHP}`;$('#pressure').style.width=`${p.charge}%`;$('#pressure-label').textContent=`${Math.floor(p.charge)}%`;$('#pressure-note').textContent=p.charge>=game.ventCost?`Vent ready · ${game.ventCost}`:'Build pressure with hits';$('#quest-title').textContent=q.title;$('#quest-detail').textContent=q.detail;if(lastQuest!==q.title+q.count){lastQuest=q.title+q.count;questUntil=now+6;}$('#quest').classList.toggle('expanded',now<questUntil);$('#hud').classList.toggle('in-combat',!!game.nearestEnemy(13));$('#equipped-name').textContent=p.attack?.name||w.name;$('#attack-symbol use').setAttribute('href','#'+(p.weapon==='shear'?'edge':p.weapon==='pike'?'lance':'hammer')+'-icon');$('#quest-count').textContent=q.count;$('#attack-name').textContent=p.weapon==='maul'?'Crush':p.weapon==='pike'?'Thrust':'Cut';$('#vent-name').textContent=w.vent;$('#attack i').style.transform=`scaleY(${p.attack?1-p.attack.age/p.attack.duration:0})`;$('#vent i').style.transform=`scaleY(${p.charge<game.ventCost?1-p.charge/game.ventCost:0})`;$('#dodge').style.opacity=p.dodge>0?'.5':'1';for(const b of $$('[data-weapon]')){b.classList.toggle('selected',b.dataset.weapon===p.weapon);b.setAttribute('aria-pressed',String(b.dataset.weapon===p.weapon));}
 ...
 const area=region(p.x,p.z);if(started&&area!==lastRegion){lastRegion=area;$('#area h2').textContent=area;areaUntil=now+3.5;}$('#area').classList.toggle('show',now<areaUntil&&!view.overview);$('#feedback').classList.toggle('show',now<feedbackUntil);$('#damage-vignette').classList.toggle('show',p.flash>0);if(now>toastUntil&&toastQueue.length){const t=toastQueue.shift();$('#toast').textContent=t.text;toastUntil=now+t.duration;}$('#toast').classList.toggle('show',now<toastUntil);$('#caption').classList.toggle('show',now<captionUntil);$('#chapter').classList.toggle('show',now<chapterUntil);if(padPresent)$('#desktop-hint').textContent='Left stick move · A strike · X vent · B evade · Y interact · LB / RB weapon';}
try{
  view=new WorldView($('#world'),game);wire();if(preview){const link=$('.opening-preview');link.textContent='Return to my main adventure';link.href='./';}window.stormEngineReady=true;$('#loading').classList.add('hidden');$('#welcome').classList.remove('hidden');$('#start').textContent=game.progress.briefed?'Continue':'Begin';
  let last=performance.now();function frame(now){try{const dt=Math.min((now-last)/1000,.05);last=now;const pad=gamepad(dt);if($('#dialogue').open){$('#speech').textContent=dialogueText.visible(now/1000);$('#continue').innerHTML=dialogueText.complete(now/1000)?(dialogueBeats.length?'Read on':'Until next time')+' <span aria-hidden="true">➜</span>':'Reveal <span aria-hidden="true">➜</span>';}
    if(!paused()){
    ...
    events();view.update(document.hidden||$('dialog[open]')?0:dt);if(now-lastUI>80){ui(now/1000);lastUI=now;}requestAnimationFrame(frame);
