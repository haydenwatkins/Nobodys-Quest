...
export const BARRIERS=[[-10,-9,1.5],[-12,-11,1.7],[12,3,1.4],[15,5,1.4],[-24,-17,1.7],[-26,-19,1.4],[22,31,1.7],[25,33,1.8],[9,-29,1.3],[-8,-30,1.4],[34,-26,1.5]].map(([x,z,radius],i)=>({x,z,radius,yaw:i*.8}));
...
const cells=new Map();for(const o of [...DECORATIONS,...BARRIERS,{x:0,z:19,radius:2}])for(let gx=Math.floor((o.x-o.radius-1)/5);gx<=Math.floor((o.x+o.radius+1)/5);gx++)for(let gz=Math.floor((o.z-o.radius-1)/5);gz<=Math.floor((o.z+o.radius+1)/5);gz++){const key=gx+','+gz;if(!cells.has(key))cells.set(key,[]);cells.get(key).push(o);}
